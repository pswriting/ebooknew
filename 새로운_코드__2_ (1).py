...
자, 이제 시작합니다."""
        ...
        def add_premium_table(doc, table_data):
            """프리미엄 베스트셀러 스타일 테이블 - 고급스럽고 세련된 디자인"""
            ...
            # 열 너비 계산 (전체 너비 14cm 기준)
            total_width = 14
            if cols == 2:
                col_widths = [Cm(4.5), Cm(9.5)]  # 첫 열 좁게
            elif cols == 3:
                col_widths = [Cm(3.5), Cm(5.25), Cm(5.25)]
            elif cols == 4:
                col_widths = [Cm(3), Cm(3.7), Cm(3.7), Cm(3.6)]
            else:
                col_widths = [Cm(total_width / cols)] * cols

            for col_idx in range(cols):
                for row in table.rows:
                    if col_idx < len(row.cells):
                        row.cells[col_idx].width = col_widths[col_idx] if col_idx < len(col_widths) else Cm(3)

            # 각 셀 스타일링
            for i, row_data in enumerate(table_data):
                row = table.rows[i]
                row.height = Cm(1.2)  # 넉넉한 행 높이
                ...
                for j, cell_text in enumerate(row_data):
                    if j < cols:
                        ...
                        # 첫 번째 열 (라벨/항목)
                            ...
                            para.alignment = WD_ALIGN_PARAGRAPH.CENTER
                            run = para.add_run(str(cell_text))
                            set_font(run, 10, bold=True, color=(30, 30, 30))
                            # 연한 베이지/그레이 배경
                            shading = OxmlElement('w:shd')
                            shading.set(qn('w:fill'), 'F5F5F0')
                            cell._tc.get_or_add_tcPr().append(shading)
                        ...
                        # 일반 내용 셀
                            ...
                            run = para.add_run(str(cell_text))
                            set_font(run, 10, color=(50, 50, 50))
                            # 흰색 배경 (깔끔하게)
                            shading = OxmlElement('w:shd')
                        ...
                        for margin_name, margin_val in [('top', '120'), ('left', '150'), ('bottom', '120'), ('right', '150')]:
...
{{
    ...
    "subtitle": "부제 (구체적 결과/약속, 15~25자)"
        ...
        outline_prompt = f"""당신은 자청입니다. 목차를 보는 순간 "이거 안 사면 손해다"라는 생각이 들게 만드세요.
...
위에서 제시된 [고유 시스템/공식]을 목차 전체에서 브랜딩하라.
시스템 이름이 목차 전체를 관통해야 한다.

[좋은 시스템 네이밍 예시]
- 인스타 수익화 → "하이 인텐트" 시스템 (구매 의도 높은 팔로워)
- 유튜브 수익화 → "딥 트래픽" 시스템 (수익 전환되는 조회수)
- 블로그 수익화 → "검색 자산" 시스템 (돈 버는 글)
- 다이어트 → "셋포인트" 시스템 (체중 설정값 리셋)

[나쁜 네이밍 - 절대 금지]
- ❌ 유치함: 머신, 엔진, 마법, 연금술, 비밀무기
- ❌ 흔함: 에버그린, 파이프라인, 패시브, 오토
- ❌ 뜻 불명확: 약자만 있고 의미 없는 것

━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
📖 목차 스토리 구조 (이 흐름 그대로)
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━

PART 1. [충격적 질문] ~해도 안 되는 사람들의 공통점
→ 독자의 현재 상황을 찌르고 공감시킴

PART 2. [시스템 발견] ~인데 ~하는 사람들의 비밀  
→ 시스템 이름 첫 등장, 호기심 폭발

PART 3. [시스템] 1단계 - [구체적 성과]를 만드는 [방법] 공식
→ 첫 번째 단계 공개

PART 4. [시스템] 2단계 - [구체적 성과]를 만드는 [방법] 공식
→ 두 번째 단계 공개

PART 5. [시스템] 3단계 - [구체적 성과]를 만드는 [방법] 공식
→ 세 번째 단계 공개

PART 6. [시스템] 완성 - [궁극적 결과]가 실현되는 시스템
→ 완성 후 얻게 되는 삶
...
### 📕 "셋포인트" 시스템
> 굶지 않고 요요 없이 10kg 빠지는 체중 리셋 공식

PART 1. 매일 굶어도 안 빠지는 사람들의 공통점
- 적게 먹으면 빠진다고? 그거 다 거짓말이다
- 운동 열심히 하는 사람이 더 안 빠지는 이유
- 나도 3년간 요요만 반복하다 깨달은 불편한 진실

PART 2. 많이 먹는데 살 안 찌는 사람들의 비밀
- 그 사람들의 식단과 생활을 전부 뜯어봤다
- 의지력과 체중은 아무 상관없다
- 셋포인트 시스템이 작동하는 단 하나의 원리

PART 3. 셋포인트 1단계 - 굶지 않아도 몸이 알아서 태우는 대사 리셋
- 적게 먹으면 대사가 망가진다
- 먹는 양 늘렸는데 오히려 빠진 이유
- 90%가 모르는 기초대사량의 진짜 비밀

PART 4. 셋포인트 2단계 - 운동 안 해도 체지방 빠지는 호르몬 설계
- 유산소 열심히 하면 오히려 망한다
- 하루 10분으로 충분한 진짜 이유
- 살 빠지는 사람과 안 빠지는 사람의 호르몬 차이

PART 5. 셋포인트 3단계 - 평생 요요 없는 체중 고정 설계
- 다이어트 성공해도 유지 못하면 의미 없다
- 뇌가 기억하는 체중을 바꾸는 법
- 3년째 유지 중인 사람들의 공통점 한 가지

PART 6. 셋포인트 완성 - 신경 안 써도 체중 유지되는 몸 시스템
- 이 시스템이 완성되면 생기는 일
- 6개월 뒤 거울 보고 소름 돋았다
- 평생 다이어트에서 벗어난 후 완전히 달라진 것들

━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
🔥 소제목 공식 (패턴 섞어서)
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━

[반전형] "~하면 무조건 망한다", "~는 전부 거짓말이다"
[숫자형] "3년 삽질 끝에 깨달은 것", "90%가 모르는 ~"
[질문형] "왜 ~해도 안 되는가", "~ 사람들의 공통점"
[비밀형] "아무도 안 알려주는 ~", "~의 진짜 비밀"
[결과형] "이것만 바꿔도 ~", "~ 후 완전히 달라진 것들"

━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
🚫 절대 금지
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━

❌ 유치한 네이밍: 머신, 엔진, 마법, 연금술, 에버그린
❌ 교과서체: 첫걸음/가이드/완벽/핵심전략/기초
❌ AI냄새: ~의 모든 것/~하는 방법/효과적인/성공적인
❌ 콜론(:) 사용 절대 금지
❌ 같은 패턴 반복
...
PART 1. [~해도 안 되는 사람들의 공통점]
- [소제목]
- [소제목]
- [소제목]

PART 2. [~인데 ~하는 사람들의 비밀]
- [소제목]
- [소제목]
- [소제목]

PART 3. [시스템명] 1단계 - [구체적 성과를 만드는 공식]
- [소제목]
- [소제목]
- [소제목]

PART 4. [시스템명] 2단계 - [구체적 성과를 만드는 공식]
- [소제목]
- [소제목]
- [소제목]

PART 5. [시스템명] 3단계 - [구체적 성과를 만드는 공식]
- [소제목]
- [소제목]
- [소제목]

PART 6. [시스템명] 완성 - [궁극적 결과가 실현되는 시스템]
- [소제목]
- [소제목]
- [소제목]
...
def regenerate_single_subtopic(chapter_name, subtopic_index, existing_subtopics):
    ...
    prompt = f"""당신은 자청입니다. 이 소제목 하나로 결제를 이끌어내세요.
...
❌ 금지: ~의 이해, ~하는 방법, 나침반, 열쇠, 마법, 효과적인, 성공적인
...
def regenerate_chapter_subtopics(chapter_name, chapter_index):
    ...
    # 챕터별 역할 정의
    chapter_roles = {
        0: "각성 - ~해도 안 되는 사람들의 공통점 찌르기",
        1: "발견 - ~인데 ~하는 사람들의 비밀 공개",
        2: "1단계 - 시스템의 첫 번째 구체적 방법",
        3: "2단계 - 시스템의 두 번째 구체적 방법",
        4: "3단계 - 시스템의 세 번째 구체적 방법",
        5: "완성 - 시스템 완성 후 얻게 되는 삶"
    }
    current_role = chapter_roles.get(chapter_index, "핵심 내용 전달")

    prompt = f"""당신은 자청입니다. 이 챕터 소제목 3개로 "이건 사야 돼" 만드세요.
...
✅ 3개 모두 다른 패턴으로:
1번: [반전형] "~하면 무조건 망한다", "~는 전부 거짓말이다"
2번: [숫자/스토리형] "3년 삽질 끝에~", "그 사람들을 전부 뜯어봤다"
3번: [비밀/결과형] "~의 진짜 비밀", "~ 후 소름 돋았다"

❌ 절대 금지:
- ~의 이해, ~하는 방법, 효과적인, 성공적인
- 나침반, 열쇠, 마법, 가이드, 완벽 정리
- 같은 패턴/비유 반복

소제목 3개만 (줄바꿈으로 구분, 기호/번호 없이):"""
...
def generate_outline(topic, persona, pains, gaps=None):
    ...
    prompt = f"""당신은 자청입니다. 목차를 보는 순간 "이거 안 사면 손해다"라는 생각이 들게 만드세요.
...
이 책만의 고유한 시스템/공식 이름을 만들어라.
목차 전체가 이 시스템을 중심으로 전개되어야 한다.
...
2. 짧은 문장 (10~20자)
3. 대화하듯 쓴다 (~했다, ~였다, ~더라)
4. 감정을 넣는다 ("미치겠더라", "소름 돋았다")
5. 독자에게 직접 말한다 ("혹시 이거, 당신 얘기 아닌가?")

━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
📊 표를 적극 활용하라
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━

비교, 단계, 수치가 나오면 반드시 마크다운 표로 정리하라.
표는 가독성을 높이고 전문성을 보여준다.
...
❌ 긴 문장 (20자 넘기지 마라)
❌ 문단을 붙여 쓰기 (한 문장마다 줄바꿈)
❌ 설명체 ("~에 대해 알아보겠습니다")
❌ AI 표현 ("중요합니다", "따라서", "결론적으로")
❌ 유치한 비유 (마법, 열쇠, 나침반, 보물)
❌ 이모지 사용
❌ 번호 나열 (1. 2. 3.)
